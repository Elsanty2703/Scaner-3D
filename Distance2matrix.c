...
void Distance2matrix(MatrixData *data) {
    ...
    switch (data->state) {
        case DATA: {
            ...
            float lectura;
            if(serial_read_float(&lectura)!=0){
                lectura = ESPERANDO_LECTURA; // Si no se pudo leer, asigna un valor de espera
            }
            if (lectura == data->lectura_anterior ||lectura <= 0) {
